...
import { Layout, Col, Button, Typography, Skeleton, Space, Input, Tooltip, Tag } from 'antd'
...
const languageSupportDocTemplateDownload = process.env.REACT_APP_LANGUAGE_SUPPORT_DOCS_CSV_DOWNLOAD
const languageAbsoluteDocumentDownload = process.env.REACT_APP_LANGUAGE_ABSOLUTE_DOCUMENT_DOWNLOAD
const Language = () => {
                ...
                return (
                    <>
                        <Tooltip title={record.language}>
                            <Text className='max-w-xs' ellipsis={{ tooltip: record.language }}>
                                {record.language}
                            </Text>
                        </Tooltip>
                    </>
                ...
                return (
                    ...
                    <>
                        <Tooltip title={record.native_name}>
                            <Text className='max-w-xs' ellipsis={{ tooltip: record.native_name }}>
                                {record.native_name}
                            </Text>
                        </Tooltip>
                    </>
    ...
    const findByPageLanguageData = (page, limit) => {
        ...
        MarketplaceServices.findByPage(languageAPI, null, page, limit, false)
            .then(function (response) {
                ...
                console.log('server Success response from language API call', response.data.data)
    ...
    const findAllSupportDocumentTemplateDownload = () => {
        MarketplaceServices.findMedia(languageSupportDocTemplateDownload, {
            ...
            .then(function (response) {
                console.log('Server Response from getStoreApi Function: ', response.data)
                  ...
                  <div className=" !mr-[10px]">
                    Download Support Document Template
                            ...
                            <Content className='!w-[20%] text-right !right-0'>
                                <Button className=' app-btn-primary' onClick={() => navigate('add_language')}>
                                    {/* <img
                    src={plusIcon}
            ...
            <Content className='!p-3 mt-[7.8rem] '>
                {isLoading ? (
                    ...
                    <Content className='p-3 text-center bg-white !rounded-md'>
                        Unfortunately, we were unable to retrieve language information. Please try again later.
                    </Content>
                ) : (
                    <Layout className=''>
                        <Content>
