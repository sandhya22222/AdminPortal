...
import { Row, Col, Typography, Input, Select, Button, Spin, Skeleton, Upload, Radio } from 'antd'
...
const EditLanguage = () => {
                ...
                <Row className='w-[50%] float-left flex items-center my-3'>
                    <Col>
                        <Link to='/dashboard/language'>
                            <ArrowLeftOutlined role={'button'} className={'ml-4 text-black text-lg'} />
                        </Link>
                    </Col>
                    <Col className='ml-4'>
    ...
    const getLanguageAPI = (page, limit) => {
        ...
        MarketplaceServices.findByPage(languageAPI, null, page, limit, false)
            .then((response) => {
                console.log('Response from  edit language server-----> ', response.data)
                let languageData = response?.data.data.filter((element) => element.id === parseInt(_id))
                ...
                if (languageData && languageData.length > 0) {
                    ...
                    copyofLanguageDetails.lang_support_docs = languageData[0].lang_support_docs_path
    ...
    const validateLanguageFieldEmptyOrNot = () => {
        ...
        // ) {
        if (languageDetails.language.trim() === '' && languageDetails.language_code.trim() === '') {
            validValues--
        ...
        } else if (languageDetails.language === '' && languageDetails.language_code !== '') {
            validValues--
            setIsLanguageFieldEmpty(true)
            toast('Please provide values for the mandatory fields', {
                autoClose: 10000,
        ...
        } else if (languageDetails.language_code === '' && languageDetails.language !== '') {
            validValues--
            setIsLanguageCodeFieldEmpty(true)
            toast('Please provide values for the mandatory fields', {
                autoClose: 10000,
            ...
            validValues--
            setIsLanguageFieldEmpty(true)
            toast(`Language must contain minimum of ${titleMinLength}, maximum of ${titleMaxLength} characters`, {
                position: toast.POSITION.TOP_RIGHT,
                type: 'error',
                autoClose: 10000,
            })
            ...
            validValues--
            setIsLanguageCodeFieldEmpty(true)
            toast(`Language code must contain minimum of 2, maximum of 5 characters`, {
                position: toast.POSITION.TOP_RIGHT,
                type: 'error',
                autoClose: 10000,
            })
        } else if (languageDetails.native_name !== null && languageDetails.native_name !== '') {
            if (
            ...
            responseLanguageData[0].lang_support_docs_path === languageDetails.lang_support_docs &&
            responseLanguageData[0].native_name === languageDetails.native_name &&
            responseLanguageData[0].writing_script_direction === languageDetails.writing_script_direction &&
    ...
    const updateEditLanguage = () => {
        ...
        MarketplaceServices.update(languageAPI, temp, { _id: parseInt(_id) })
            .then(function (response) {
                ...
                setLanguageDetails(copyofLanguageDetails)
                console.log('response of language data', response.data[0].lang_support_docs)
            ...
            .catch((error) => {
                ...
                if (error && error.response && error.response.status === 401) {
                    ...
                    if (fileData) {
                        if (fileValue !== '.csv')
                            toast("Invalid file extension, only '.csv' extension is supported.", {
                                position: toast.POSITION.TOP_RIGHT,
                                type: 'error',
                                autoClose: 10000,
                            })
        ...
        MarketplaceServices.remove(languageAPI, { _id: parseInt(_id) })
            ...
            .catch((error) => {
                ...
                if (error && error.response && error.response.status === 401) {
                    ...
                    toast('Language cannot be deleted since there is a reference in the store', {
                        position: toast.POSITION.TOP_RIGHT,
                        type: 'error',
                        autoClose: 10000,
                    })
                ...
                title={
                    <Content className='flex'>
                        <Content className='flex text-left self-center items-center pr-3'>
                            <Link to='/dashboard/language'>
                                <ArrowLeftOutlined role={'button'} className={'text-black text-lg -translate-y-1'} />
                            </Link>
                                                    ...
                                                    <Col span={8} className='pl-2'>
                                                        <Content className='my-3'>
                                                            ...
                                                            <Input
                                                                ...
                                                                onBlur={() => {
                                                                    const trimmed = languageDetails.language_code.trim()
                                                                    const trimmedUpdate = trimmed.replace(/\s+/g, ' ')
                                                        ...
                                                        <Content className='my-3'>
                                                            ...
                                                            <Input
                                                                ...
                                                                onBlur={() => {
                                                                    ...
                                                                    const trimmedUpdate = trimmed.replace(/\s+/g, ' ')
                                                                    languageHandler('language_regex', trimmedUpdate)
                                                                }}
                                                            />
                                                        </Content>
                                                    </Col>
                                                    <Col span={8} className='pl-2'>
                                                        <Content className='my-3'>
                                                            ...
                                                            <Input
                                                                ...
                                                                onBlur={() => {
                                                                    const trimmed = languageDetails.native_name.trim()
                                                                    const trimmedUpdate = trimmed.replace(/\s+/g, ' ')
