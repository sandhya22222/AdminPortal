  ...
  const handleValidationGroup = () => {
    ...
    if (groupName === "") {
      ...
      MarketplaceToaster.showToast(
        util.getToastObject(
          `${t("common:values_for_mandatory_fields")}`,
          "error"
        )
      );
      ...
      count--;
      MarketplaceToaster.showToast(
        util.getToastObject(`${t("common:no_change_detected")}`, "warning")
      );
    }
    if (count === 1) {
      if (groupNameProps === "") {
      ...
      <Spin tip="Please wait" spinning={isLoading}>
        ...
        <Content>
          ...
          <Content className="float-right">
            <Button
              className="mr-2 app-btn-secondary"
