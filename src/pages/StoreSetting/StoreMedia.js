...
const bannerImagesLengthDisplay = process.env.REACT_APP_BANNER_IMAGES_MAX_LENGTH
...
const StoreMedia = ({ type, title, getImageData, setGetImageData }) => {
    ...
    const findAllWithoutPageStoreImagesApi = () => {
        MarketplaceServices.findAllWithoutPage(storeImagesAPI, {
            ...
            .catch((error) => {
                setGetImageData([])
                console.log('Get error response of Store setting Images--->', error.response)
            })
    }
    ...
    const saveStoreLogoImageCall = (fileValue) => {
        ...
        MarketplaceServices.save(storeImagesAPI, formData)
            .then((response) => {
                ...
                console.log('Server Success Response From storeImagePostCall', response.data)
                setGetImageData([response.data])
                if (response.data && response.data.store_logo_path) {
                    setStoreLogo([
                        {
            ...
            .catch((error) => {
                setImageUploading(false)
                setStoreLogo([])
                const filteredArray = bannerFileList.filter((item) => Number.isInteger(item.uid))
    ...
    const updateStoreLogoImageCall = (fileValue) => {
        ...
        MarketplaceServices.update(storeImagesAPI, formData)
            .then((response) => {
                ...
                console.log('Server Success Response From storeImagePutCall', response.data)
            })
            .catch((error) => {
                setStoreLogo([])
                setImageUploading(false)
    ...
    const handleRemove = (file) => {
        ...
        return new Promise((resolve, reject) => {
            confirm({
                ...
                okText: 'Ok',
                content: 'Are you absolutely sure you want to delete the image? This action cannot be undone.',
                onOk: () => {
                    let dataObject = {}
                        ...
                        <>
                            <Upload
                                ...
                                <Button
                                    icon={<UploadOutlined />}
                                    disabled={
                                        bannerFileList && bannerFileList.length < bannerImagesLengthDisplay
                                            ? false
                                            : true
                                    }>
