...
const DynamicTable = ({ tableComponentData }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const defaultSortArray =
        tableComponentData.sorting_settings.sorting_data &&
        tableComponentData.sorting_settings.sorting_data.length > 0 &&
        tableComponentData.sorting_settings.sorting_data.filter(
          (item) => item.default === true
        );
      if (defaultSortArray.length > 0) {
        const value =
          defaultSortArray &&
          defaultSortArray.length > 0 &&
          defaultSortArray[0].title;
        const sortType =
          defaultSortArray &&
          defaultSortArray.length > 0 &&
          defaultSortArray[0].sortType;
        const sortKey =
          defaultSortArray &&
          defaultSortArray.length > 0 &&
          defaultSortArray[0].sortKey;
  ...
  const handleSearchBoxChange = (e) => {
    ...
    if (
      dataSource.search_settings.search_data.length &&
      dataSource.search_settings.search_data.length > 0
    ) {
    ...
    if (checkBoxData && checkBoxData.length > 0) {
      ...
      if (uniqueCheckBoxTypes.length > 0) {
        ...
        for (var i = 0; i < uniqueCheckBoxTypes.length; i++) {
          ...
          const typeData = checkBoxData.filter(
          ...
          for (var j = 0; j < typeData.length; j++) {
            ...
            const filterResult = filteredData.filter(
      ...
      if (
        ...
        if (selectedSortingType === "default") {
          ...
          setTableData(
            handleSorting(selectedSortingKey, result, selectedSortingType)
          );
        }
      } else {
        setTableData(result);
  ...
  const handleCheckBoxClick = (e, type) => {
    ...
    const itemIndex =
      ...
      checkBoxData.findIndex(
        (item) =>
          item.id === id &&
          item.name === name &&
          item.type === typeofCheckBoxData
      );
                        ...
                        <div>
                          ...
                          >
                            {dataSource.sorting_settings.sorting_data &&
                              dataSource.sorting_settings.sorting_data.length >
                                0 &&
                      ...
                      {dataSource.filter_settings.is_enabled ? (
                        ...
                        >
                          <DropdownToggle className="mr-1">
                            {dataSource.filter_settings.filter_title !==
                            null ? (
                              <Button>
                                {dataSource.filter_settings.filter_title}
                              </Button>
